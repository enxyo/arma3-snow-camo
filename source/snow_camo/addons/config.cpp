...
class CfgPatches
{
	class a3_snow_camo
	{
		...
		requiredAddons[] = {"A3_Characters_F_BLUFOR","A3_Characters_F_Exp","A3_Characters_F_Mark"};
...
class CfgVehicles
{
	...
	class B_ghillie_arctic_1_F: B_ghillie_base_F       			// Define of a new class, which parameters are inherited from B_Soldier_base_F, with exception of those defined below.
    {
        ...
        sensitivity = 2.5;                  // How likely this character spots enemies when controlled by AI.
        threat[] = {1, 1, 0.8};             // Multiplier of the cost of the character in the eyes of soft, armoured and air enemies.
        model = "\A3\Characters_F_Mark\BLUFOR\b_fullghillie_f.p3d";       	 // The path to the model this character uses.
        uniformClass = "U_B_FullGhillie_arctic_1_F";                         // This links this soldier to a particular uniform. For the details, see below.
		hiddenSelections[] = {"camo","camo3","camo4"};                       // List of model selections which can be changed with hiddenSelectionTextures[] and hiddenSelectionMaterials[] properties. If empty, model textures are used.
        hiddenSelectionsTextures[] = {"\snow_camo\addons\data\ghillie_coverall_arctic.paa","\snow_camo\addons\data\ghillie_threads_arcitc.paa","\snow_camo\addons\data\ghillie_threads_5lod_arctic.paa"};        // The textures for the selections defined above. If empty, no texture is used.
        canDeactivateMines = true;              // Can this character deactivate mines?
        engineer = true;                        // Can this character repair vehicles?
        attendant = 1;                          // Can this character heal soldiers?
        icon = "iconManEngineer";               // If a character has a special role, a special icon shall be used.
